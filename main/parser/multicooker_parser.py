...
from main.models import Multicooker, MulticookerComment, MulticookerGrade
...
def get_otzyvy(otz_url, ttl, multicooker):
    driver.get(otz_url)
    sleep(3)
    ...
    for _div in _divs:
        ...
        if not MulticookerComment.objects.filter(text=str(comment)).exists():
            multicooker.multicookercomment_set.create(text=str(comment), grade=grade)
        multicooker.save()
...
def get_data(drvr):
    ...
    for div in divs:
        ...
        if not Multicooker.objects.filter(title=title_final).exists():
            mltcooker_model = Multicooker.objects.create(title=title_final)
            multicooker_models.append(mltcooker_model)
            get_harakteristiki(harakteristiki_url, mltcooker_model)
            get_otzyvy(otzyvy_url, title_final, mltcooker_model)
            mltcooker = {'title': title_final, 'item_url': _url, 'image_url': ''}
            multicooker.append(mltcooker)
    ...
    for span in img_spans:
        ...
        imgs.append(src)
    for i in range(len(multicooker)):
        multicooker_models[i].img_url = imgs[i]
        multicooker_models[i].save()
        multicooker[i]['image_url'] = imgs[i]
    print(f'Search Мультиварки...')
    print(f'  Result ({len(multicooker)}):')
    with open("rbt.txt", "w", encoding='windows-1251') as res_file:
        for f in multicooker:
...
def run_parser():
    Multicooker.objects.all().delete()
    MulticookerComment.objects.all().delete()
    MulticookerGrade.objects.all().delete()
    url = 'https://msk.rbt.ru/cat/kuhonnaya_tehnika/multivarki/'
    driver.get(url)
